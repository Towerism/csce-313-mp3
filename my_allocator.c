...
void * free_list_allocator( int total_mem_request ){
  int offset = 4;//some num that = the size of the linked list
//log takes a double but we want it rounded to an int
  ...
  for(int order = high_order; order > 0; --order){
    struct Linked_list* ll = (memory_pool + offset);
    *ll = list_create();
    ll->size = pow(2, order) * b_b_s;
    //allocate as many high-order blocks as possible
    while(remaining_mem >= ll->size){
        list_push_back(ll, memory_pool + remaining_mem);
        remaining_mem -= ll->size;
    }
